...
from .models import Customer, Category, Product, ProductPrice, Order, OrderItem, OrderStatusHistory
...
class ProductResource(resources.ModelResource):
    ...
    class Meta:
        model        = Product
        fields       = ('id', 'category', 'name', 'description', 'image_file', 'is_active',
                        'unit_label', 'weight_grams', 'price', 'is_available')
        export_order = fields
...
class OrderAdmin(admin.ModelAdmin):
    list_display       = ('pk', 'customer', 'subtotal', 'delivery_charge', 'total', 'status', 'created_at')
    ...
    fields             = ('customer', 'status', 'subtotal', 'delivery_charge', 'total', 'created_at', 'updated_at')

    def save_model(self, request, obj, form, change):
        super().save_model(request, obj, form, change)
        if change and 'status' in form.changed_data:
            OrderStatusHistory.objects.create(order=obj, status=obj.status, amount=obj.total)
